...
type parse_proc_t = (node:parse_node) => boolean;
...
class parse_node_generator {
	private _node: parse_node;

	constructor(node: parse_node) {
		this._node = node;
	}

	static node(state: parse_state): parse_node_generator {
		return new parse_node_generator(new parse_node(state));
	}
	public node(state: parse_state): parse_node_generator {
		return new parse_node_generator(new parse_node(state));
	}
	 ...
	 * 保持しているノードを渡して生成完了
	 */
	public gen(): parse_node {
		return this._node;
	}

	static seq(nodes: parse_node[]): parse_node_generator {
		let [first, ...remain] = nodes;
		let temp = new parse_node_generator(first)
		if (remain) temp.seq(remain);
		return temp;
	}
	public seq(node_gen: parse_node[]) : parse_node_generator {
		return this;
	}
	public or(node_gen: parse_node[]): parse_node_generator {
		return this;
	}
}
...
let pngen = parse_node_generator;
...
export class parse_node {
	...
	private _parse_check_tbl: { [key: string]: parse_proc_t };		// パース遷移チェックテーブル
	 ...
	 * @param flag 
	 */
	public err_stop(flag: boolean): parse_node {
		this._err_stop = flag;
		return this;
	}
	 ...
	 * @param curr 
	 */
	private _parse_proc_root(curr: parse_node): boolean {
	...
	private _parse_proc_hub(curr: parse_node): boolean {
		let result: boolean = false;
		// hubはchildを1つだけ持っている
	 ...
	 * @param curr 
	 */
	private _parse_proc_impl_check_proc_else(node: parse_node, else_node: parse_node|null): boolean {
		let result: boolean = false;
		// ノード遷移チェック
		result = this._parse_check(node);
		if (result) {
	 ...
	 * @param curr 
	 */
	private _parse_proc_impl_check_proc(node: parse_node): boolean {
		let result: boolean = false;
		// ノード遷移チェック
		result = this._parse_check(node);
		if (result) {
	 ...
	 * @param next 
	 */
	private _parse_check_impl_check(next: parse_node): parse_check_result {
		...
		if (next._check != null) {
			...
			if (check_result) {
				...
				result = 'check_ok';
				// 状態遷移チェック結果ログ更新
				this._last_checked_idx = parse_node.LAST_CHECKED_SELF;
			} else {
				// 判定NG
				result = 'check_ng';
			}
	...
	private _parse_check_impl_seq(next: parse_node): parse_check_result {
		let result: parse_check_result = 'check_undef';
		let check_result: boolean;
		// 状態遷移チェック
		// 状態遷移チェック結果ログを最初に初期化しておく
	...
	private _parse_check_impl_child_all(next: parse_node): parse_check_result {
		let result: parse_check_result = 'check_undef';
		let check_result: boolean;
	...
	private _parse_check_impl_child_head(next: parse_node): parse_check_result {
		let result: parse_check_result = 'check_undef';
		let check_result: boolean;
		let child_idx: number = 0;
		// 状態遷移チェック
	...
	private _parse_check_root(next: parse_node): boolean {
		let result: boolean = false;
		// 'node'と同じ処理
		result = this._parse_check_node(next);
		return result;
	}
	private _parse_check_node(next: parse_node): boolean {
		let result: boolean = false;
	...
	private _parse_check_hub(next: parse_node): boolean {
		let result: boolean = false;
		// 'node'と同じ処理
		result = this._parse_check_node(next);
		return result;
	}
	private _parse_check_seq(next: parse_node): boolean {
		let result: boolean = true;
		// 'node'と同じ処理
		result = this._parse_check_node(next);
		return result;
	}
	private _parse_check_or(next: parse_node): boolean {
		let result: boolean = false;
		let check_result: parse_check_result;
		// 自ノードcheck判定
		check_result = this._parse_check_impl_check(next);
	...
	private _parse_check_opt(next: parse_node): boolean {
		let result: boolean = false;
		// 'node'と同じ処理
		result = this._parse_check_node(next);
		return result;
	}
	private _parse_check_many(next: parse_node): boolean {
		let result: boolean = false;
		// 'node'と同じ処理
		result = this._parse_check_node(next);
		return result;
	}
	private _parse_check_many1(next: parse_node): boolean {
		let result: boolean = false;
		// 'node'と同じ処理
		result = this._parse_check_node(next);
		return result;
	}
	private _parse_check_else(next: parse_node): boolean {
		let result: boolean = true;
		return result;
	}
...
export class parser {
	...
	private make_parse_tree() {
		...
		// (6.7.8) initializer-list:
		let pn_init_list: parse_node = new parse_node('initializer-list', this.ev_null, this.at_null);
	
		let pn_root: parse_node = new parse_node('root', this.ev_null, this.at_null);
		let pn_prepro: parse_node = new parse_node('pp-directive', this.ev_pp, this.at_null);				// preprocessing-directive
		let pn_extern_decl: parse_node = new parse_node('statement', this.ev_null, this.at_null);			// external-declaration
		let pn_decl_spec: parse_node = new parse_node('declaration-specifiers', this.ev_decl_spec, this.at_null);		// declaration-specifiers
		let pn_init_decl_list: parse_node = new parse_node('statement', this.ev_null, this.at_null);		// init-declarator-list
		let pn_declarator: parse_node = new parse_node('statement', this.ev_null, this.at_null);			// declarator
